...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        while (index > 1) {
            Node nodeChild = getNode(index);
            Node nodeParent = getNode(parentIndex(index));
            if (nodeChild.myPriority < nodeParent.myPriority) {
    ...
    public void changePriority(T item, double priority) {
        int prevIndex = findItem(item);
        if (prevIndex == 0) return;
        Node prevNode = getNode(prevIndex);
        if (prevNode.myPriority <= priority) {
            prevNode.myPriority = priority;
            sink(prevIndex);
        } else {
            prevNode.myPriority = priority;
            swim(prevIndex);
    ...
    /**
     * Helper function to find the index of the given item.
